...
def templateMatchingTM_COEFF_NORMED(accuracy,myImage, myTemplate):
    #the most accurate method found
    #loads the RGB image
    img_rgb = cv2.imread(myImage)
    #transforms it into grayscale
    img_gray = cv2.cvtColor(img_rgb, cv2.COLOR_BGR2GRAY)
    #loads the template
    template = cv2.imread(myTemplate,0)
...
templateMatchingTM_COEFF_NORMED(0.85, 'google.png','go.png')
